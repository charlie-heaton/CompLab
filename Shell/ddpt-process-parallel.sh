...
echo Extracting $END lines

START=1

awk -v start="$START" -v end="$END" 'NR>=start&&NR<=end' $HOME/$2 > swapfile
...
awk 'NR>1 {print $4}' mode.energy >> energy_30_$END
echo "$END" >> energy_sum_$END
echo $(sed 's/$/+/' energy_30_$END) 0 | bc >> energy_sum 
